  ...
  },
  {
    ...
    subMenus: [
      ...
      },
      {
        ...
        href: "/admin/ceremony-management/ceremony-participants",
      },
      {
        label: "Paid Participants",
        icon: <UserCheck className="w-4 h-4" />,
        href: "/admin/ceremony-management/paid-participants",
      },
    ],
  },
  {
...
export default function AdminSidebar() {
  ...
  const [openMenus, setOpenMenus] = useState(() => {
    ...
    menuItems.forEach((item) => {
      if (item.subMenus) {
        initial[item.label] = item.subMenus.some(
          (sub) =>
                          ...
                          <SidebarMenuButton
                            isActive={sub.href
                              ...
                              ?.every(
                                (seg, i) =>
                                  seg ===
                                  router.pathname?.toLowerCase()?.split("/")[i]
                              )}
                ...
                <Link href={item.href} className="block">
                  <SidebarMenuButton
                    isActive={item.href
                      ...
                      ?.every(
                        (seg, i) =>
                          seg === router.pathname?.toLowerCase()?.split("/")[i]
                      )}
      ...
      <SidebarFooter>
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton
              asChild
              className="text-red-500 cursor-pointer hover:text-red-600 hover:bg-transparent active:bg-transparent active:text-red-500"
            >
