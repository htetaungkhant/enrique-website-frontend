...
const urlToFile = async (url, filename) => {
  try {
    ...
    toast({
      title: "Error",
      description:
        "Failed to load image. Please try again or upload a new image.",
      variant: "destructive",
    });
    return null;
  }
  ...
  gallery: z
    ...
    .refine(
      (files) =>
        files.every(
          (file) =>
            (file instanceof File && file.size <= MAX_IMAGE_SIZE_BYTES) ||
            typeof file === "string"
        ),
      `Image size must not exceed ${MAX_IMAGE_SIZE_MB}MB`
    )
  ...
  extraDetails: z
    .array(
      z.object({
        ...
        points: z
          .array(z.string().trim().min(1, "Note shouldn't be empty"))
...
export function EditCeremonyForm({ initialData }) {
  ...
  const [gallery, setGallery] = useState(
    initialData?.gallery?.map((img) => ({ id: img.id, image: img.image })) || []
  );
    ...
    const convertImages = async () => {
      ...
      if (initialData?.image?.image) {
        const imageFile = await urlToFile(
          ...
          `ceremony-image-${initialData.id}.${initialData.image.image
            .split(".")
            .pop()}`
        );
        if (imageFile) {
      ...
      if (initialData?.hosts?.length > 0) {
        const hostPromises = initialData.hosts.map(async (host, index) => {
          if (host.image?.image) {
            const imageFile = await urlToFile(
              host.image.image,
              `host-image-${index}-${initialData.id}.${host.image.image
                .split(".")
                .pop()}`
            );
            return {
      ...
      if (initialData?.gallery?.length > 0) {
        const galleryPromises = initialData.gallery.map(async (img, index) => {
          if (img.image) {
            const imageFile = await urlToFile(
              img.image,
              `gallery-image-${index}-${initialData.id}.${img.image
                .split(".")
                .pop()}`
            );
            return {
        ...
        const updatedGallery = (await Promise.all(galleryPromises)).filter(
          Boolean
        );
  ...
  const handleGalleryImageChange = async (e) => {
    ...
    if (validFiles.length !== files.length) {
      toast.error(
        `Some images were skipped. Images must be under ${MAX_IMAGE_SIZE_MB}MB and in jpg, jpeg, png, or webp format.`
      );
    }
  ...
  const removeGalleryImage = (index) => {
    ...
    if (removedImage?.id) {
      ...
      const currentFormGallery = form.getValues("gallery") || [];
      const updatedFormGallery = currentFormGallery.filter(
        (file) => file !== removedImage.image
      );
  ...
  const handleAddHost = () => {
    ...
    if (newHostImage) {
      ...
      if (editingHost !== null) {
        ...
        const updatedHosts = [
          ...hosts,
          { name: trimmedName, image: newHostImage },
        ];
        setHosts(updatedHosts);
        form.setValue("hosts", updatedHosts);
      }
  ...
  const handleRemoveHost = (index) => {
    ...
    if (updatedHosts.length === 0) {
      form.setError("hosts", {
        type: "custom",
        message: "Please add at least one host",
      });
    }
  };
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (
        ...
        // Handle gallery images
        const existingGalleryImages = gallery.filter(
          (g) => typeof g.image === "string"
        );
        ...
        formData.append(
          ...
          JSON.stringify(orderedDeletedImages)
        );
      } else {
      ...
      if (!response.ok) {
        if (response.status === 413) {
          try {
            ...
            toast.error(
              errorResponse?.error ||
                `Image size exceeds limit. Please upload one by one.`
            );
          } catch (error) {
    ...
    <Form {...form}>
      ...
      <Dialog open={hostDialogOpen} onOpenChange={setHostDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingHost !== null ? "Edit Host" : "Add New Host"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <FormItem>
              ...
              <div
                onClick={() =>
                  document.getElementById("hostImageInput").click()
                }
                className="mt-2 cursor-pointer flex items-center justify-center border-2 border-dashed border-gray-300 rounded-lg p-4 hover:border-gray-400 transition-colors"
              >
                {newHostImage ? (
                  <div className="relative w-32 h-32">
                    <Image
                      src={
                        newHostImage instanceof File
                          ? URL.createObjectURL(newHostImage)
                          : newHostImage
                      }
                      alt="Host preview"
                      fill
