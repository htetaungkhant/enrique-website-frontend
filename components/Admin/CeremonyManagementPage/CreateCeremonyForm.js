  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error(
          responseJson?.error || "Failed to create ceremony. Please try again."
        );
      }
    } catch (error) {
      toast.error(
        error.message || "Failed to create ceremony. Please try again."
      );
    } finally {
  ...
  return (
    <Form {...form}>
      <form
        ...
        className="mx-auto space-y-8 bg-white p-6 rounded-lg"
      >
        <FormField
          control={form.control}
          name="markAsSold"
