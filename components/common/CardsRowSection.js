...
const CardsRowSection = ({ title, description, cardList }) => {
    return (
        <UPSection className="flex flex-col gap-6">
            {title && <h2 className="text-white text-2xl inter-font font-light lg:text-4xl" dangerouslySetInnerHTML={{ __html: title }}></h2>}
                            ...
                            cardList.map((item, index) => (
                                <Card className="flex-1" badgeText={item.badgeText} key={index}>
                                    <div className="flex flex-col gap-4 inter-font text-[#022645]">
                                        {item.title && <h2 dangerouslySetInnerHTML={{ __html: item.title }} className="font-normal text-xl md:text-2xl lg:text-3xl"></h2>}
