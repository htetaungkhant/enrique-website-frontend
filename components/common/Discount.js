...
const Discount = ({ discountUsers, discountPercent, onSubmissionSuccess }) => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            toast.error(error?.response?.data?.message || "Early Bird Discount is already full!");
        } finally {
    ...
    return (
        <>
            {/* Overlay */}
            {showDiscount && (<div className="fixed z-900 inset-0 bg-black opacity-20"></div>)}
            {
                showDiscount === "intro" && (
                    <div className="lg:max-h-100 max-md:w-full max-w-[300px] md:max-w-180 xl:max-w-120 flex z-900 fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-gradient-to-r from-[#CDFFD8] to-[#94B9FF] overflow-hidden rounded-xl shadow">
                        ...
                        {/* Close Button */}
                        <IoMdCloseCircle size={24} onClick={() => setShowDiscount("")} className="absolute top-2 right-2 z-50 text-gray-200 hover:text-gray-50 cursor-pointer transition" />

                        <div className="p-3 lg:px-6 lg:py-6 md:w-1/2 flex flex-col justify-center gap-3 lg:gap-6">
                            <h2 className="text-[#212A63] font-black text-xl lg:text-2xl">Unlock a 10% Early Bird Discount!</h2>
                            <div className="text-[10px] lg:text-xs font-medium">
                                <p>Be among the FIRST 3 to register for our 7-Day Luxury Healing Retreat in Ibiza and receive an exclusive 10% off.</p>
                                <br />
