...
const LoginForm = ({ toggleLoginSignup }) => {
  ...
  const onClickGuestCheckout = async (e) => {
    e.preventDefault();

    const query = { ...router.query, checkout: "guest-ceremony" };
    delete query.auth;
    router.replace({
      pathname: router.pathname,
      query: query,
  ...
  return (
    <form
      ...
      noValidate
      className="px-3 flex-1 flex flex-col gap-3 justify-center max-lg:text-xs text-sm text-[#848484]"
    >
      ...
      <div className="mt-4 xl:mt-8 mx-auto w-full lg:max-w-85 flex flex-col gap-5">
        ...
        <p className="text-center text-[#2D3748]">
          ...
          <span
            className="font-bold text-[#212A63] cursor-pointer"
