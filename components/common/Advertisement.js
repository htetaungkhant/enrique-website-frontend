...
const Advertisement = () => {
  ...
  useEffect(() => {
    ...
    const hasSubmitted =
      sessionStorage.getItem("hasSubmittedNewsletter") === "true";
    ...
    const startTimer = () => {
      if (!showAdvertisement && !hasSubmitted) {
        ...
        }, 120000); // Show every 2 minute (120000ms)
      }
    };
  ...
  return (
    <>
      {/* Overlay */}
      {showAdvertisement && (
        <div className="fixed z-1000 inset-0 bg-black opacity-20"></div>
      )}
      {showAdvertisement === "intro" && (
        <div className="lg:max-h-100 max-md:w-full max-w-[300px] md:max-w-180 xl:max-w-120 flex z-1000 fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-gradient-to-r from-[#CDFFD8] to-[#94B9FF] overflow-hidden rounded-xl shadow">
          ...
          <div className="p-4 lg:px-5 lg:py-6 md:w-1/2 flex flex-col justify-center gap-1.5 lg:gap-3">
            <h2 className="text-[#212A63] font-black text-xl lg:text-2xl">
              Is Your Soul Prepared for the Journey?
            </h2>
            <p className="lg:text-lg font-semibold">
              Are you ready to take the first step toward exploring deeper
              self-awareness and spiritual growth. Sign in to our Newsletters
              for more updates!
            </p>
            <Image
              width={259}
