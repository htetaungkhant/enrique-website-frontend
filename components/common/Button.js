	...
	const content = (
		<>
			<FaRegUserCircle
				size={s.iconSize}
				className={cn(
					`p-1 border-2 text-white rounded-full w-auto transition duration-300`,
					outline ? "border-white bg-transparent" : "border-black bg-black",
					s.icon,
					iconClassName
				)}
			/>
...
const Button = ({
	...
	return (
		<button
			className={cn("p-2 rounded-full bg-[#212A63] text-white font-medium cursor-pointer hover:shadow-lg disabled:bg-[#212a63be] disabled:cursor-not-allowed", className)}
			{...props}
		>
			{children}
		</button>
