...
import {
  ...
  getCoursesByUser,
} from "@/lib/inhouseAPI/course-route";
...
export async function getServerSideProps(context) {
  try {
    ...
    const course = await getCourseDetailsByTitle({
      ...context.req,
...
const CourseDetails = ({ course, isAlreadyEnrolled }) => {
  ...
  useEffect(() => {
    const handleStripeCallback = async () => {
      ...
      router.replace(`/course-offerings/${router.query.courseId}`, undefined, {
        shallow: true,
      });
  ...
  useEffect(() => {
    const restartCeremonyCheckout = sessionStorage.getItem(
      "restartCeremonyCheckout"
    );
    if (
      session &&
      !session.validationFailed &&
      restartCeremonyCheckout === "true"
    ) {
      sessionStorage.removeItem("restartCeremonyCheckout");
            ...
            <div>
              <div className="pt-3 flex flex-wrap gap-6 lg:gap-10">
                <div className="flex items-center gap-3 font-medium">
                  <Image
                    src={course.createdBy?.image?.image}
                    width={100}
                    height={100}
                    alt="avator"
