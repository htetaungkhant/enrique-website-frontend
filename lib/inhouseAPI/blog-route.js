...
export async function getBlogDetails(req) {
  ...
  try {
    const response = await inhouseApi.get(`${blog_route}/get-by-id/${id}`);
    if (response.status === 200) {
      return response.data;
    } else {
      console.error(`Unexpected response status: ${response.status}`);
...
export async function getBlogDetailsByTitle(req) {
  ...
  try {
    ...
    if (Array.isArray(blogs) && blogs?.length > 0) {
      ...
      if (blog) {
        ...
        console.error(`Blog with title ${title} not found`);
        return null;
      }
    } else {
    ...
    console.error("Error fetching blog details by title:", error);
    return null;
  }
}
