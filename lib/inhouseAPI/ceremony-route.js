...
export async function getCeremonyDetails(req) {
    ...
    try {
        ...
        const { ceremonies } = await getCeremonies({ ...req, body });
...
export async function getCeremoniesByUser(req) {
...
export async function updateCeremony(req) {
    const { id, title, hostNames, gallery, images, image, location, startDate, endDate, sessionOverview, extraDetails, price } = req.body;
    if (!id) return;
    const token = await getInHouseToken(req);
