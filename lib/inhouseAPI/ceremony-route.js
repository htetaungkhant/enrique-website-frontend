...
// res.setHeader('Set-Cookie', 'next-auth.session-token.admin=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT; HttpOnly; SameSite=Lax');
...
export async function getCeremonyDetailsByTitle(req) {
  ...
  try {
    ...
    const { ceremonies, discountUsers } = !token
      ? await getCeremonies({ ...req, body })
      : await getAllCeremonies({ ...req, body });
    ...
    if (Array.isArray(ceremonies) && ceremonies?.length > 0) {
      const ceremony = ceremonies.find(
        (c) =>
          c.title?.replaceAll(/\s+/g, "-").toLowerCase() === title.toLowerCase()
      );
      if (ceremony) {
        return { ceremony, discountUsers };
      } else {
        console.error(`Ceremony with title ${title} not found`);
        return null;
      }
    } else {
      console.error("No ceremonies found");
    ...
    console.error("Error fetching ceremony details by title:", error);
    return null;
  }
}
...
export async function createCeremony(req) {
  ...
  formData.append("markAsSold", req.body.markAsSold[0]);
  ...
  if (Array.isArray(req.body.gallery) && req.body.gallery.length > 0) {
    req.body.gallery.forEach((galleryImage) => {
      ...
      const galleryFileBlob = new Blob([galleryFileContent], {
        type: galleryImage.mimetype,
      });
      formData.append(
        "gallery",
        galleryFileBlob,
        galleryImage.originalFilename
      );
    });
  }
  ...
  if (Array.isArray(req.body.images) && req.body.images.length > 0) {
    req.body.images.forEach((image) => {
      ...
      const imageFileBlob = new Blob([imageFileContent], {
        type: image.mimetype,
      });
...
export async function updateCeremony(req) {
  ...
  try {
    const response = await inhouseApi.put(
      `${ceremony_route}/update`,
      formData,
      {
        headers: {
          "Content-Type": "multipart/form-data",
...
export async function registerCeremony(req) {
  const { id, orderId, stripeMetadata } = req.body;
  ...
  try {
    const body = {
      id,
      orderId,
      stripeMetadata: stripeMetadata || {}, // all stripe data
    };
    const response = await inhouseApi.post(`${ceremony_route}/register`, body, {
      headers: {
        Authorization: `Bearer ${userToken}`,
      },
    });
...
export async function getPaidCeremonyParticipants(req) {
  const token = await getInHouseToken(req);
  ...
  try {
    const body = {
      page: req.body?.page || 1,
      limit: req.body?.limit || 10,
      sort: req.body?.sortByDate || "asc",
    };
    const response = await inhouseApi.post(
      `${ceremony_route}/all-registered`,
      body,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export default {
  ...
  getAllCeremonies,
  getCeremonyDetails,
  getCeremonyDetailsByTitle,
  getRegisteredCeremoniesByUser,
  createCeremony,
  updateCeremony,
  registerCeremony,
  deleteCeremony,
  getPaidCeremonyParticipants,
};
