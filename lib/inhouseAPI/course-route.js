...
export async function getCourseDetailsByTitle(req) {
  ...
  try {
    ...
    if (Array.isArray(courses) && courses?.length > 0) {
      ...
      if (course) {
        return await getCourseDetails({ ...req, body: { id: course.id } });
      } else {
    ...
    console.error("Error fetching course details by title:", error);
    return null;
  }
}
...
export async function getCoursesByUserId(req) {
  const { userId, page, limit, sortByDate } = req.body;
  if (!userId) return;
...
export async function createCourse(req) {
  ...
  const courseImageFileBlob = new Blob([courseImageFileContent], {
    type: courseImageFile.mimetype,
  });
  formData.append(
    "courseImage",
    courseImageFileBlob,
    courseImageFile.originalFilename
  );

  const createdByImageFile = req.body.createdByImage[0];
  const createdByImageFileContent = fs.readFileSync(
    createdByImageFile.filepath
  );
  const createdByImageFileBlob = new Blob([createdByImageFileContent], {
    type: createdByImageFile.mimetype,
  });
  formData.append(
    "createdByImage",
    createdByImageFileBlob,
    createdByImageFile.originalFilename
  );
...
export async function registerCourse(req) {
  ...
  try {
    ...
    const response = await inhouseApi.post(`${course_route}/register`, body, {
      headers: {
        Authorization: `Bearer ${userToken}`,
      },
    });
    if (response.status === 200) {
      console.log("success register course");
      return response.data;
    } else {
      console.error(`Unexpected response status: ${response.status}`);
      return null;
